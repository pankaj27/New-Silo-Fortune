...
@section('title', \App\CPU\translate('Trading Breed Add'))
...
<div class="content container-fluid">
    ...
    <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
        <h2 class="h1 mb-0 d-flex align-items-center gap-2">
            ...
            {{\App\CPU\translate('breed')}} {{\App\CPU\translate('Setup')}}
        </h2>
    </div>
    <!-- End Page Title -->
        ...
        <div class="col-md-12">
            <div class="card mb-3">
                <div class="card-body" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                    <form action="{{route('admin.trading-breed.add-new')}}" method="post" enctype="multipart/form-data">
                        @csrf
                            ...
                            <div class="col-md-6">
                                @foreach(json_decode($language) as $lang)
                                    <div class="form-group {{$lang != $default_lang ? 'd-none':''}} lang_form"
                                            id="{{$lang}}-form">
                                        <label for="name" class="title-color">{{ \App\CPU\translate('Breed_name')}}<span class="text-danger">*</span> ({{strtoupper($lang)}})</label>
                                        <input type="text" name="name[]" class="form-control" id="name" value="" placeholder="{{\App\CPU\translate('Ex')}} : {{\App\CPU\translate('Breed_name')}}" {{$lang == $default_lang? 'required':''}}>
                                    </div>
                                    <input type="hidden" name="lang[]" value="{{$lang}}">
                                @endforeach
                                <div class="form-group">
                                    <label for="name" class="title-color">{{ \App\CPU\translate('Breed_Logo')}}<span class="text-danger">*</span></label>
