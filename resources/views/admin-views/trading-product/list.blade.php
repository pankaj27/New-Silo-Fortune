...
@section('title', \App\CPU\translate('Trading Product List'))
...
<div class="content container-fluid">
    ...
    <div class="mb-3">
        <h2 class="h1 mb-0 text-capitalize d-flex gap-2">
            ...
            @if($type == 'in_house')
                {{\App\CPU\translate('Trading_In_House_Product_List')}}
            @elseif($type == 'seller')
                {{\App\CPU\translate('Trading_Seller_Product_List')}}
            @endif
                        ...
                        <div class="col-lg-8 mt-3 mt-lg-0 d-flex flex-wrap gap-3 justify-content-lg-end">
                            ...
                            <div>
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    <li><a class="dropdown-item" href="{{route('admin.trading-product.export-excel',['in_house', ''])}}">Excel</a></li>
                                    <div class="dropdown-divider"></div>
                                </ul>
                            </div>
                            <!--<a href="{{route('admin.trading-product.stock-limit-list',['in_house'])}}" class="btn btn-info">
                                <span class="text">{{\App\CPU\translate('Limited Sotcks')}}</span>
                            </a>-->
                            @endif
                            @if (!isset($request_status))
                                <a href="{{route('admin.trading-product.add-new')}}" class="btn btn--primary">
                                    <i class="tio-add"></i>
                                    <span class="text">{{\App\CPU\translate('Add_New_Product_Trading')}}</span>
                                </a>
                            @endif
                        </div>
                    </div>
                    ...
                    <table id="datatable" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};" class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100">
                        ...
                        @foreach($pro as $k=>$p)
                            <tr>
                                ...
                                <td>
                                    <a href="{{route('admin.trading-product.view',[$p['id']])}}" class="media align-items-center gap-2">
                                        <img src="{{\App\CPU\ProductManager::trading_product_image_path('thumbnail')}}/{{$p['thumbnail']}}"
                                ...
                                <td>
                                    <div class="d-flex justify-content-center gap-2">
                                        ...
                                        <a class="btn btn-outline-info btn-sm square-btn" title="View" href="{{route('admin.trading-product.view',[$p['id']])}}">
                                            <i class="tio-invisible"></i>
                                        </a>
                                        <a class="btn btn-outline--primary btn-sm square-btn"
                                            title="{{\App\CPU\translate('Edit')}}"
                                            href="{{route('admin.trading-product.edit',[$p['id']])}}">
                                            <i class="tio-edit"></i>
                                        </a>
                                    ...
                                    <form action="{{route('admin.trading-product.delete',[$p['id']])}}"
...
@push('script')
    ...
    <script>
        ...
        $(document).on('change', '.status', function () {
            ...
            $.ajax({
                url: "{{route('admin.trading-product.status-update')}}",
                method: 'POST',
                data: {
                    id: id,
                    status: status
        ...
        function featured_status(id) {
            ...
            $.ajax({
                url: "{{route('admin.trading-product.featured-status')}}",
                method: 'POST',
                data: {
                    id: id
                },
