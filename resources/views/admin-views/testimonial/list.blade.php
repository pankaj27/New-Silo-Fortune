    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 d-flex gap-2">
                ...
                {{\App\CPU\translate('Trading_testimonial')}} {{\App\CPU\translate('List')}}
                <span class="badge badge-soft-dark radius-50 fz-14">{{ $testimonial->total() }}</span>
            </h2>
        </div>
        <!-- End Page Title -->
                ...
                <div class="card">
                    ...
                    <div class="px-3 py-4">
                        <div class="row g-2 flex-grow-1">
                            ...
                            <div class="col-sm-4 col-md-6 col-lg-4 d-flex justify-content-end">
                                <a href="{{ route('admin.trading-testimonial.add-new') }}">
                                    <button type="button" class="btn btn-primary">
                                        
                                        {{\App\CPU\translate('add_testimonial')}}
                        ...
                        <div class="table-responsive">
                            <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                ...
                                <tr>
                                    ...
                                    <th>{{ \App\CPU\translate('testimonial_user_images')}}</th>
                                    <th>{{ \App\CPU\translate('testimonial_name')}}</th>
                                    <th>{{ \App\CPU\translate('testimonial_description')}}</th>
                                ...
                                @foreach($testimonial as $k=>$b)
                                    <tr>
                                        <td>{{$b['id']}}</td>
                                        <td>
                                            <img class="rounded avatar-60"
                                                 onerror="this.src='{{asset('public/assets/back-end/img/160x160/img2.jpg')}}'"
                                                 src="{{asset('storage/app/public/testimonial')}}/{{$b['image']}}">
                                        </td>
                                        <td>{{$b['name']}}
                                            <br>
                                            <span class="label label-info">{{$b['designation']}}</span>
                                        </td>
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-center gap-2">
                                                ...
                                                href="{{route('admin.trading-testimonial.edit',[$b['id']])}}">
                                                <i class="tio-edit"></i>
                                                </a>
...
@push('script')
    <script>
        $(document).on('click', '.delete', function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{route('admin.trading-testimonial.delete')}}",
                        method: 'POST',
                        data: {id: id},
                        success: function () {
                            toastr.success('{{ \App\CPU\translate('Testimonial_deleted_successfully')}}');
                            location.reload();
                        }
                    });
                }
        ...
        $(document).on('change', '.status', function () {
            ...
            $.ajax({
                url: "{{route('admin.trading-testimonial.status-update')}}",
                method: 'POST',
                data: {
                    id: id,
                    status: status
