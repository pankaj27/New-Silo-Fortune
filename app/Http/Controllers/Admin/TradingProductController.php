...
class TradingProductController extends BaseController
{
    public function add_new()
    {
        $cat = DB::table('trading_categories')->where(['parent_id' => 0])->get();
        $br = Breed::orderBY('name', 'ASC')->get();
        $brand_setting = BusinessSetting::where('type', 'product_brand')->first()->value;
        $digital_product_setting = BusinessSetting::where('type', 'digital_product')->first()->value;
        return view('admin-views.trading-product.add-new', compact('cat', 'br', 'brand_setting', 'digital_product_setting'));
    }

    public function featured_status(Request $request)
    {
        $product = TradingProduct::find($request->id);
    ...
    public function approve_status(Request $request)
    {
        ...
        return redirect()->route('admin.trading-product.list', ['seller', 'status' => $product['request_status']]);
    }
    ...
    public function deny(Request $request)
    {
        ...
        return redirect()->route('admin.trading-product.list', ['seller', 'status' => 2]);
    }

    public function view($id)
    {
        $product = TradingProduct::with(['reviews'])->where(['id' => $id])->first();
        $reviews = Review::where(['product_id' => $id])->whereNull('delivery_man_id')->paginate(Helpers::pagination_limit());
        return view('admin-views.trading-product.view', compact('product', 'reviews'));
    }
        ...
        if ($request->ajax()) {
            ...
            if ($request->file('images')) {
                foreach ($request->file('images') as $img) {
                    $product_images[] = ImageManager::upload('trading-product/', 'png', $img);
                }
                $p->images = json_encode($product_images);
            }
            $p->thumbnail = ImageManager::upload('trading-product/thumbnail/', 'png', $request->image);

            if($request->product_type == 'digital' && $request->digital_product_type == 'ready_product') {
                $p->digital_file_ready = ImageManager::upload('trading-product/digital-product/', $request->digital_file_ready->getClientOriginalExtension(), $request->digital_file_ready);
            }

            $p->meta_title       = $request->meta_title;
            $p->meta_description = $request->meta_description;
            $p->meta_image       = ImageManager::upload('trading-product/meta/', 'png', $request->meta_image);
            ...
            foreach ($request->lang as $index => $key) {
                if ($request->name[$index] && $key != 'en') {
                    array_push($data, array(
                        'translationable_type' => 'App\Model\TradingProduct',
            ...
            return redirect()->route('admin.trading-product.list', ['in_house']);
        }
    }
    ...
    function list(Request $request, $type)
    {
        ...
        if ($type == 'in_house') {
            $pro = TradingProduct::where(['added_by' => 'admin']);
        } else {
            $pro = TradingProduct::where(['added_by' => 'seller'])->where('request_status', $request->status);
        }
        ...
        return view('admin-views.trading-product.list', compact('pro', 'search', 'request_status', 'type'));
    }
    ...
    public function export_excel(Request $request, $type){
        $products = TradingProduct::when($type == 'in_house', function ($q){
    ...
    public function updated_product_list(Request $request)
    {
        ...
        if ($request->has('search')) {
            $key = explode(' ', $request['search']);
            $pro = TradingProduct::where(['added_by' => 'seller'])
            ...
            $pro = TradingProduct::where(['added_by' => 'seller'])->where('is_shipping_cost_updated',0);
        }
        $pro = $pro->orderBy('id', 'DESC')->paginate(Helpers::pagination_limit())->appends($query_param);

        return view('admin-views.trading-product.updated-product-list', compact('pro', 'search'));
    }
    ...
    public function stock_limit_list(Request $request, $type)
    {
        ...
        $search = $request['search'];
        if ($type == 'in_house') {
            $pro = TradingProduct::where(['added_by' => 'admin', 'product_type'=>'physical']);
        } else {
            $pro = TradingProduct::where(['added_by' => 'seller', 'product_type'=>'physical'])->where('request_status', $request->status);
        }
        ...
        $pro = $pro->orderBy('id', 'DESC')->paginate(Helpers::pagination_limit())->appends(['status' => $request['status']])->appends($query_param);
        return view('admin-views.trading-product.stock-limit-list', compact('pro', 'search', 'request_status', 'sort_oqrderQty', 'stock_limit'));
    }
    ...
    public function update_quantity(Request $request)
    {
        ...
        $product = TradingProduct::find($request['product_id']);
        if ($stock_count >= 0) {
    ...
    public function status_update(Request $request)
    {
        ...
        $product = TradingProduct::where(['id' => $request['id']])->first();
        $success = 1;
    ...
    public function updated_shipping(Request $request)
    {
        ...
        $product = TradingProduct::where(['id' => $request['product_id']])->first();
        if($request->status == 1)
        {
    ...
    public function get_categories(Request $request)
    {
        ...
        */
        ...
        $cat = DB::table('trading_categories')->where(['parent_id' => $request->parent_id])->get();
        $res = '<option value="' . 0 . '" disabled selected>---Select---</option>';
        foreach ($cat as $row) {
            if ($row->id == $request->sub_category) {
                $res .= '<option value="' . $row->id . '" selected >' . $row->name . '</option>';
    ...
    public function sku_combination(Request $request)
    {
        ...
        $combinations = Helpers::combinations($options);
        return response()->json([
            'view' => view('admin-views.trading-product.partials._sku_combinations', compact('combinations', 'unit_price', 'colors_active', 'product_name'))->render(),
        ]);
    }
    ...
    public function get_variations(Request $request)
    {
        $product = TradingProduct::find($request['id']);
        return response()->json([
            'view' => view('admin-views.trading-product.partials._update_stock', compact('product'))->render()
        ]);
    }

    public function edit($id)
    {
        $product = TradingProduct::withoutGlobalScopes()->with('translations')->find($id);
        $product_category = json_decode($product->category_ids);
        $product->colors = json_decode($product->colors);
        $categories =DB::table('trading_categories')->where(['parent_id' => 0])->get();
        $sub_categories =DB::table('trading_categories')->where(['parent_id' => $product_category[0]->id])->get();
        $br = Breed::orderBY('name', 'ASC')->get();
        $brand_setting = BusinessSetting::where('type', 'product_brand')->first()->value;
        $digital_product_setting = BusinessSetting::where('type', 'digital_product')->first()->value;
        ...
        return view('admin-views.trading-product.edit', compact('categories','sub_categories', 'br', 'product', 'product_category','brand_setting','digital_product_setting'));
    }

    public function update(Request $request, $id)
    {

        $product = TradingProduct::find($id);
        $validator = Validator::make($request->all(), [
            'name'                  => 'required',
            'category_id'           => 'required',
            'product_type'          => 'required',
        ...
        if ($request->ajax()) {
            ...
            if ($request->file('image')) {
                $product->thumbnail = ImageManager::update('trading-product/thumbnail/', $product->thumbnail, 'png', $request->file('image'));
            }

            if($request->product_type == 'digital') {
                if($request->digital_product_type == 'ready_product' && $request->hasFile('digital_file_ready')){
                    $product->digital_file_ready = ImageManager::update('trading-product/digital-product/', $product->digital_file_ready, $request->digital_file_ready->getClientOriginalExtension(), $request->file('digital_file_ready'));
                }elseif(($request->digital_product_type == 'ready_after_sell') && $product->digital_file_ready){
                    ImageManager::delete('trading-product/digital-product/'.$product->digital_file_ready);
            ...
            if ($request->file('meta_image')) {
                $product->meta_image = ImageManager::update('trading-product/meta/', $product->meta_image, 'png', $request->file('meta_image'));
            }
            ...
            Toastr::success('Trading Product updated successfully.');
            return back();
        }
    }
    ...
    {
        ImageManager::delete('/trading-product/' . $request['image']);
        $product = TradingProduct::find($request['id']);
        $array = [];
        ...
        TradingProduct::where('id', $request['id'])->update([
    ...
    public function delete($id)
    {
        ...
        $translation = Translation::where('translationable_type', 'App\Model\TradingProduct')
        ...
        foreach (json_decode($product['images'], true) as $image) {
            ImageManager::delete('/trading-product/' . $image);
        }
        ImageManager::delete('/trading-product/thumbnail/' . $product['thumbnail']);
        $product->delete();

        FlashDealTradingProduct::where(['product_id' => $id])->delete();
    ...
    {
        return view('admin-views.trading-product.bulk-import');
    }
    ...
    {
        $products = TradingProduct::where(['added_by' => 'admin'])->get();
        //export from product
        $storage = [];
        foreach ($products as $item) {
            $category_id = 0;
    ...
    public function barcode(Request $request, $id)
    {
        ...
        $product = TradingProduct::findOrFail($id);
        $limit =  $request->limit ?? 4;
        return view('admin-views.trading-product.barcode', compact('product', 'limit'));
    }
    
    public function storeTradingProduct(Request $request){
        ...
        $p->is_pregnent = $request->is_pregnent;
        $p->milking_per_day = $request->milking_perday;
        $p->lactation_number = $request->lactation_number;
        $p->seller_id = $request->seller_id;
        $p->seller_address = $request->seller_address;
        $p->seller_contact_no = $request->sller_contact;
        ...
        if ($request->ajax()) {
            ...
            $p->meta_title       = $request->name;
            $p->meta_description = $request->description;
            $p->meta_image       = ImageManager::upload('trading-product/meta/', 'png', $request->image);
    ...
    public function updateNew(Request $request, $id)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($request->file('image')) {
                $product->meta_image = ImageManager::update('trading-product/meta/', $product->thumbnail, 'png', $request->file('image'));
            }
