...
class ProductController extends BaseController
{
    ...
    {
        $cat = DB::table('shipping_addresses')->where(['customer_id' => $request->user_id])->get();
        $res = '<option value="' . 0 . '" disabled selected>---Select---</option>';
        foreach ($cat as $row) {
            
                $res .= '<option value="' . $row->id . '">' . $row->address.' '.$row->city.' '.$row->zip. '</option>';
    ...
    public function get_user_location_phone(Request $request)
    {
        $cat = DB::table('shipping_addresses')->where(['id' => $request->address_id])->get();
        //$res = '<option value="' . 0 . '" disabled selected>---Select---</option>';
        foreach ($cat as $row) {
