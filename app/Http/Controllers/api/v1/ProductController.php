...
class ProductController extends Controller
{
    ...
    public function get_latest_trading_products(Request $request)
    {
        $products = ProductManager::get_latest_trading_products($request['limit'], $request['offset']);
       // $products = DB::table('trading_products')->offset($request['offset'])->limit($request['limit'])->get();
        $products['products'] = Helpers::trading_product_data_formatting($products['products'], true);
        return response()->json($products, 200);
    }
    
    public function get_top_rated_trading_products(Request $request)
    {
        $products = ProductManager::get_top_rated_trading_products($request['limit'], $request['offset']);
        $products['products'] = Helpers::product_data_formatting($products['products'], true);
        return response()->json($products, 200);
    }
    
    public function get_best_sellings_trading(Request $request)
    {
        $products = ProductManager::get_best_selling_trading_products($request['limit'], $request['offset']);
        $products['products'] = Helpers::product_data_formatting($products['products'], true);

        return response()->json($products, 200);
    }
    
    public function get_discounted_trading_product(Request $request)
    {
        $products = ProductManager::get_discounted_trading_product($request['limit'], $request['offset']);
        $products['products'] = Helpers::product_data_formatting($products['products'], true);
        return response()->json($products, 200);
    }
    
    public function get_trading_product($slug)
    {
        $product = TradingProduct::with(['reviews.customer'])->where(['slug' => $slug])->first();
        if (isset($product)) {
            $product = Helpers::trading_product_data_formatting($product, false);
    ...
    public function trading_social_share_link($product_slug)
    {
        $product = TradingProduct::where('slug', $product_slug)->first();
        $link = route('product', $product->slug);
        try {
    ...
    public function get_related_trading_products($id)
    {
        if (TradingProduct::find($id)) {
            $products = ProductManager::get_related_trading_products($id);
            $products = Helpers::trading_product_data_formatting($products, true);
            return response()->json($products, 200);
        }
        return response()->json([
            'errors' => ['code' => 'product-001', 'message' => translate('Product not found!')]
