...
class ProductController extends Controller
{
    ...
    public function upload_trading_digital_product(Request $request){
    ...
    public function upload_trading_images(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'image' => 'required',
            'type' => 'required|in:product,thumbnail,meta',
        ...
        $path = $request['type'] == 'product' ? '' : $request['type'] . '/';
        $image = ImageManager::upload('trading-product/' . $path, 'png', $request->file('image'));

        return response()->json(['image_name' => $image, 'type' => $request['type']], 200);
    }
    
    public function add_new_trading(Request $request)
    {
        $data = Helpers::get_user_seller_by_token($request);
        $datt = Helpers::default_lang();
        
         print_r(gettype($request->lang));
        ...
        $product = new TradingProduct();
        $product->user_id = $seller->id;
        $product->added_by = "seller";

        //$product->name = $request->name[array_search(Helpers::default_lang(), $request->lang)];
        //$product->slug = Str::slug($request->name[array_search(Helpers::default_lang(), $request->lang)], '-') . '-' . Str::random(6);
        $productname = explode("[",$request->name);
       $newprod_name = explode("]",$productname[1]);
       $originalprod_name = $newprod_name[0];
        
        $product->name = $originalprod_name;
        ...
        //if ($request->has('colors_active') && $request->has('colors') && count(json_encode($request->colors)) > 0) 

        if ($request->has('colors_active') && $request->has('colors')) {
            $product->colors = $request->product_type == 'physical' ? json_encode($request->colors) : json_encode([]);
        } else {
            $colors = [];
            $product->colors = $request->product_type == 'physical' ? json_encode($colors) : json_encode([]);
       ...
       // }
        ...
        $product->save();
        $data = [];
        foreach ((array)$request->lang as $index => $key) {
            if ($request->name[$index] && $key != Helpers::default_lang()) {
                array_push($data, array(
