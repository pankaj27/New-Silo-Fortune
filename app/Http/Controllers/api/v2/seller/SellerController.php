...
use App\Model\Shop;
use App\Model\TradingShop;
use App\Model\TradingSeller;
use App\Model\TradingSellerWallet;
use App\User;
...
class SellerController extends Controller
{
    ...
    public function shop_info_trading(Request $request)
    {
        $data = Helpers::get_user_seller_by_token($request);
        $seller = $data['data'];
        if ($data['success'] == 1 && !empty($seller)) {
            $seller = $data['data'];
            //$product_ids = TradingProduct::where(['user_id' => $seller['id'], 'added_by' => 'seller'])->pluck('id')->toArray();
            $product_ids = DB::table('trading_products')->where(['user_id' => $seller->id, 'added_by' => 'seller'])->pluck('id')->toArray();
          // $shop = TradingShop::where(['seller_id' => $seller['id']])->first();
            $shop = DB::table('trading_shops')->where(['seller_id' => $seller->id])->get();
            $shop['rating'] = round(Review::whereIn('product_id', $product_ids)->avg('rating'), 3);
            $shop['rating_count'] = Review::whereIn('product_id', $product_ids)->count();
       ...
       //$product_ids = DB::table('trading_products')->where(['user_id' => $seller[0]->id, 'added_by' => 'seller'])->pluck('id')->toArray();
    ...
    //========================================================
    public function seller_info_trading(Request $request)
    {
        $data = Helpers::get_user_seller_by_token($request);
       ...
       // TradingSeller::with(['wallet'])->withCount(['product', 'orders'])->where(['id' => $seller['id']]
       //leftjoin('trading_seller_wallets as tsw', 'tsw.seller_id', '=', $seller[0]->id)
       //Seller::with(['wallet'])->withCount(['product', 'orders'])->where(['id' => $seller['id']]

       // return response()->json(DB::table('trading_sellers')->where(['id' => $seller[0]->id])->first(), 200);
       
        return response()->json(TradingSeller::with(['wallet'])->withCount(['product', 'orders'])->where(['id' => $seller[0]->id])->first(), 200);
        
        
    }
    
    public function trading_shop_product_reviews(Request $request)
    {
        $data = Helpers::get_user_seller_by_token($request);
