...
class ProductManager
{
    ...
    public static function trading_product_image_path($image_type)
    {
        $path = '';
        if ($image_type == 'thumbnail') {
            $path = asset('storage/app/public/trading-product/thumbnail');
        } elseif ($image_type == 'product') {
            $path = asset('storage/app/public/trading-product');
        }
        return $path;
    }
    ...
    public static function get_trading_seller_all_products($seller_id, $limit = 10, $offset = 1)
    {
        $paginator = TradingProduct::with(['rating'])
            ->where(['user_id' => $seller_id, 'added_by' => 'seller'])
            ->latest()
            ->paginate($limit, ['*'], 'page', $offset);
        /*$paginator->count();*/
    ...
    public static function get_latest_trading_products($limit = 10, $offset = 1)
    {
        //$paginator = TradingProduct::with(['rating'])->latest()->paginate($limit, ['*'], 'page', $offset);
        $paginator = TradingProduct::all();
        /*$paginator->count();*/
        return [
    ...
    public static function get_top_rated_trading_products($limit = 10, $offset = 1)
    {
        
        $reviews = TradingProduct::with(['rating'])->active()
            ->withCount(['reviews'])->orderBy('reviews_count', 'DESC')
            ->paginate($limit, ['*'], 'page', $offset);
    ...
    public static function get_best_selling_trading_products($limit = 10, $offset = 1)
    {
        //change reviews to rattings
        $paginator = OrderDetail::with('product.rating')
            ->whereHas('product', function ($query) {
    ...
    public static function get_discounted_trading_product($limit = 10, $offset = 1)
    {
        //change review to ratting
        $paginator = TradingProduct::with(['rating'])->active()->where('discount', '!=', 0)->latest()->paginate($limit, ['*'], 'page', $offset);
        return [
            'total_size' => $paginator->total(),
            'limit' => (int)$limit,
            'offset' => (int)$offset,
    ...
    public static function get_related_trading_products($product_id)
    {
        $product = TradingProduct::find($product_id);
        return TradingProduct::active()->with(['rating'])->where('category_ids', $product->category_ids)
            ->where('id', '!=', $product->id)
            ->limit(10)
            ->get();
    }
