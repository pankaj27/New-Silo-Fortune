...
class TradingProductManager
{
    ...
    {
        return TradingProduct::active()->with(['rating'])->where('id', $id)->first();
    }

    public static function get_latest_products($limit = 10, $offset = 1)
    {
        $paginator = TradingProduct::active()->with(['rating'])->latest()->paginate($limit, ['*'], 'page', $offset);
        /*$paginator->count();*/
        return [
    ...
    public static function get_featured_products($limit = 10, $offset = 1)
    {
        ...
        $paginator = TradingProduct::with(['rating'])->active()
            ->where('featured', 1)
    ...
    public static function search_products($name, $limit = 10, $offset = 1)
    {
        ...
        $paginator = TradingProduct::active()->with(['rating'])->where(function ($q) use ($key) {
    ...
    public static function translated_product_search($name, $limit = 10, $offset = 1)
    {
        ...
        $paginator = TradingProduct::WhereIn('id', $product_ids)->paginate($limit, ['*'], 'page', $offset);
    ...
    {
        $paginator = TradingProduct::active()->with(['rating'])
