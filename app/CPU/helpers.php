...
class Helpers
{
    ...
    public static function rating_count_trading($product_id, $rating)
    {
        return TradingReview::where(['product_id' => $product_id, 'rating' => $rating])->whereNull('delivery_man_id')->count();
    }
    ...
    public static function trading_set_data_format($data)
    {
        try {
            $variation = [];
            $data['category_ids'] = json_decode($data->category_ids);
    ...
    public static function trading_product_data_formatting($data, $multi_data = false)
    {
        $storage = [];
        if ($multi_data == true) {
            foreach ($data as $item) {
                $storage[] = Helpers::trading_set_data_format($item);
            }
            $data = $storage;
        } else {
            $data = Helpers::trading_set_data_format($data);;
        }
    ...
    public static function get_user_seller_by_token($request)
    {
        $data = '';
        $success = 0;
        ...
        $token = explode(' ', $request->header('authorization'));
        if (count($token) > 1 && strlen($token[1]) > 30) {
            //$seller = TradingSeller::where(['auth_token' => $token['1']])->first();
            $seller = DB::table('trading_sellers')->where('auth_token',$token[1])->first();
